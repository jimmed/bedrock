...
const parseServerProperties = (file, { generatorSettings = false } = {}) => {
  const properties = file
    ...
    .reduce((props, [key, value]) => {
      ...
      props[ccKey] = parser(value, ccKey);
      return props;
    }, {});
...
const writeServerProperties = async (cwd, content) =>
  await fs.writeFile(
    getServerPropertiesPath(cwd),
    formatServerProperties(content),
    "utf8"
  );
...
const updateServerProperties = async (cwd, update = x => x) =>
  await writeServerProperties(
    cwd,
    await update(await readServerProperties(cwd))
  );
